/// Implementing SHA-256 based on this tutorial on Github:
/// 
...
/// Message is padded according to SHA256 specification
fn pad_message(message: &[u8]) -> Vec<u8> {
    let mut padded = message.to_vec();

    let original_len_bits = (message.len() * 8) as u64;

    padded.push(0x80); // Append '1' bit

    while (padded.len() * 8) % 512 != 448 {
        padded.push(0x00); // Pad with zeros until the length is congruent to 448 mod 512
    }

    padded.extend(&original_len_bits.to_be_bytes()); // Append the original length in bits

    padded
}

fn process_chunks(padded_message: &[u8]) -> [u32; 8] {
    let mut h = H; // Initialize hash values
